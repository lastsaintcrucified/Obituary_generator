...
export default function ObituaryOutput() {
  const { obituaries, freePreviewUsed, selectedObituaryIndex, setSelectedObituaryIndex } = useObituaryStore()
  ...
  const generatePDF = () => {
    ...
    // Add a footer
    doc.setFontSize(10)
    doc.text("Generated with Obituary Generator", 105, 280, { align: "center" })
      ...
      <Card className="h-full">
        <CardContent className="p-6 flex flex-col items-center justify-center min-h-[400px] text-center">
          <p className="text-slate-500 mb-4">
            Fill out the form and click "Generate Obituary" to see your drafts here.
          </p>
          <div className="w-16 h-16 rounded-full bg-slate-100 flex items-center justify-center mb-4">
            <Edit className="h-8 w-8 text-slate-400" />
          </div>
          <p className="text-sm text-slate-400">You'll receive three unique drafts to choose from.</p>
        </CardContent>
      </Card>
    )
  }
        ...
        <CardContent className="p-6">
          ...
          >
            ...
            {obituaries.map((obituary, index) => (
              <TabsContent key={`draft${index + 1}`} value={`draft${index + 1}`} className="space-y-4">
                {isEditing ? (
                  <Textarea
            ...
            <div className="flex justify-between mt-4">
              <Button variant="outline" onClick={handleEditToggle} className="text-slate-700">
                {isEditing ? "Save Edits" : "Edit Text"}
                <Edit className="ml-2 h-4 w-4" />
              </Button>

              <Button onClick={handleDownload} className="bg-slate-800 hover:bg-slate-700" disabled={isLoading}>
                {isLoading ? (
                  <>
