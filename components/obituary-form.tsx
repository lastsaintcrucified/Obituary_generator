...
export default function ObituaryForm() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      if (!data.obituaries || !Array.isArray(data.obituaries) || data.obituaries.length !== 3) {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6" id="obituary-form">
        <FormField
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>Choose the tone that best reflects the deceased's values.</FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-800 rounded-md p-3 text-sm">
            ...
            <p>{error}</p>
            <p className="mt-2 text-xs">Please check that your OpenRouter API key is properly configured.</p>
          </div>
        )}
        <Button type="submit" className="w-full bg-slate-800 hover:bg-slate-700" disabled={isGenerating}>
          {isGenerating ? (
            <>
